...
OUTPUT_ZOOM = 1      # how much to zoom output relative to *original* image 相对于*原始*图像可缩放多少输出
OUTPUT_DPI = 300         # just affects stated DPI of PNG, not appearance 仅影响PNG的指定DPI，不影响外观
REMAP_DECIMATE =8   # downscaling factor for remapping image 缩小比例以重新映射图像
ADAPTIVE_WINSZ =55   # window size for adaptive threshold in reduced px 自适应阈值的窗口大小，以减少的px为单位 不能换

TEXT_MIN_WIDTH =25   # min reduced px width of detected text contour最小检测到的文字轮廓的px宽度
TEXT_MIN_HEIGHT =5     # min reduced px height of detected text contour 检测到的文字轮廓的最小降低px高度
TEXT_MIN_ASPECT =1.5 # filter out text contours below this w/h ratio 过滤出低于此w / h比率的文本轮廓
TEXT_MAX_THICKNESS =30  # max reduced px thickness of detected text contour检测到的文字轮廓最大减少的px厚度

EDGE_MAX_OVERLAP = 1.0   # max reduced px horiz. overlap of contours in span最大减少的像素水平。 跨度中的轮廓重叠
EDGE_MAX_LENGTH = 50 # max reduced px length of edge connecting contours边缘连接轮廓的最大减少px长度
EDGE_ANGLE_COST = 10.0   # cost of angles in edges (tradeoff vs. length)边角的成本（权衡与长度）
EDGE_MAX_ANGLE =7.5    # maximum change in angle allowed between contours轮廓之间允许的最大角度变化

RVEC_IDX = slice(0, 3)   # index of rvec in params vector rvec在参数向量中的索引 slice 返回一个切片对象。
TVEC_IDX = slice(3, 6)   # index of tvec in params vector tvec在参数向量中的索引
CUBIC_IDX = slice(6, 8)  # index of cubic slopes in params vector 参数向量中三次边坡的索引

SPAN_MIN_WIDTH = 30    # minimum reduced px width for span 跨度的最小减小的px

# 宽度
SPAN_PX_PER_STEP =30  # reduced px spacing for sampling along spans 减少px间距以进行跨距采样
FOCAL_LENGTH = 1.2       # normalized focal length of camera 相机的标准化焦距
...
# nice color palette for visualizing contours, etc.漂亮的调色板，用于可视化轮廓等
CCOLORS = [
    (255, 0, 0),
    (255, 63, 0),
    (255, 127, 0),
...
# default intrinsic parameter matrix 默认内在参数矩阵
K = np.array([
    [FOCAL_LENGTH, 0, 0],
    [0, FOCAL_LENGTH, 0],
...
def debug_show(name, step, text, display):
    ...
    if DEBUG_OUTPUT != 'file':
        ...
        cv2.putText(image, text, (16, height-16),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0,
                    (255, 255, 255), 1, cv2.LINE_AA)##cv2.LINE_AA 为抗锯齿，这样看起来会非常平滑。
...
def get_default_params(corners, ycoords, xcoords):
    ...
    # our initial guess for the cubic has no slope 我们对立方的初步猜测没有坡度
    cubic_slopes = [0.0, 0.0]

    # object points of flat page in 3D coordinates 平面页面在3D坐标中的目标点
    ...
    # correspondences 从四个2D到3D点对应关系估计旋转和平移
...
#box(9,1) 高9宽1
        mask = cv2.dilate(mask, box(9, 1))##膨胀操作

        if DEBUG_LEVEL >= 3:
            debug_show(name, 0.2, 'dilated', mask)

        mask = cv2.erode(mask, box(1, 3))##腐蚀操作
...
def generate_candidate_edge(cinfo_a, cinfo_b):
    ...
    # right of left endpoint of a. 我们想要b的左边（所以a的后继者将是b，b的前任者将是a），确保b的右端点在a的左端点的右边。
...
def sample_spans(shape, spans):
    ...
    for span in spans:
        ...
        org_points.append(contour_points)

        contour_points = pix2norm(shape, contour_points)

        span_points.append(contour_points)

    return span_points, org_points

def show_img(img, span_points):
    # span_points = span_points.reshape((-1, 2))
    # print(np.shape(span_points))
    # print(span_points)
...
def keypoints_from_samples(name, small, pagemask, page_outline,
    ...
    for points in span_points:
        ...
        #
        #
        # cv2.circle(small, cntr, 3, (255, 0, 255), 2)  # 在PCA中心位置画一个圆圈
        # p1 = (cntr[0] + 0.02 * eigenvectors[0, 0] * eigenvalues[0, 0],
        #       cntr[1] + 0.02 * eigenvectors[0, 1] * eigenvalues[0, 0])
        # p2 = (cntr[0] - 0.02 * eigenvectors[1, 0] * eigenvalues[1, 0],
        #       cntr[1] - 0.02 * eigenvectors[1, 1] * eigenvalues[1, 0])
        # drawAxis(small, cntr, p1, (0, 255, 0), 1)  # 绿色，较长轴
        # drawAxis(small, cntr, p2, (255, 255, 0), 1)  # 黄色
        # print(p1)

        weight = np.linalg.norm(points[-1] - points[0])##范数是对向量（或者矩阵）的度量，是一个标量（scalar）：
...
def get_page_dims(corners, rough_dims, params):
    ...
    res = scipy.optimize.minimize(objective, dims, method='SLSQP')
    dims = res.x
...
def remap_image(name, img, small, page_dims, params):
    ...
    remappedfile = name + '_remapped.png'
    cv2.imwrite(remappedfile,remapped)
    # remapped.save(threshfile, dpi=(OUTPUT_DPI, OUTPUT_DPI))
    img=cv2.resize(img,(remapped.shape[1],remapped.shape[0]))
    imgg=np.hstack([img,remapped])
    cv2.imshow('imgg',imgg)
    cv2.waitKey(0)
    result=name+'_oringin_correct.png'
    cv2.imwrite(result,imgg)
...
if __name__ == '__main__':
    ...
    # img_files = get_files(r'‪D:\pycharm\work\page_dewrap\page_dewarp-master\test\linguistics_thesis_a.jpg')
    # print(img_files)
    # imgfile=(r'D:\pycharm\work\page_dewrap\page_dewarp-master\linguistics_thesis_a.jpg')
    imgfile=(r"D:\pycharm\work\page_dewrap\page_dewarp-master\example_input\TEST\test2.png")
    # imgfile=(r'‪D:\pycharm\work\page_dewrap\page_dewarp-master\example_input\TEST\test1.jpg')
    # exit(0)
    outfiles = []
    # for imgfile in img_files:
    #     if not os.path.exists(imgfile):
    #         exit(0)
    img = cv2.imread(r'D:\pycharm\work\page_dewrap\page_dewarp-master\example_input\TEST\test2.png')
    ...
    span_points, org_points = sample_spans(small.shape, spans)
    show_img(small,org_points)
