...
for i in range(0,224):
    ...
    w.append(w1)
    # print(h[0],w[0])
    h2, _,_ = img_array2[i].shape
    h_n.append(h2)
    w_n = w
    if w[i] / h[i] >= w_n[i] / h_n[i]:
        img_new = cv.resize(img_array1[i], (w_n[i], int(w_n[i] * h[i]/ w[i])))
    else:
        img_new = cv.resize(img_array1[i], (int(h_n[i] * w[i] / h[i]), h_n[i]))
    img_new_array.append(img_new)

    img_generate=np.hstack((img_array2[i],img_new_array[i]))
    img_generate_array.append(img_generate)
    cv.imwrite(r'D:\pycharm\work\rh/%d.png' % (i), img_generate, [int(cv.IMWRITE_JPEG_QUALITY), 95])


    # img_generate.append(img_generate1)
